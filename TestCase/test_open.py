...
class Test_Ad_Open():
    ...
    def test_ad_config(self, case_data, api_response):
        ...
        check_and_assert(mango_conf.get_adtemplate, api_response, '$..template',"返回的template与配置不一致", "未下发模版")
        check_and_assert(mango_conf.get_clicktype, api_response, '$..clicktype',"返回的点击触发类型与配置不一致",
                         "mango配置了clicktype但实际未下发")
        check_and_assert(mango_conf.get_isdeeplink, api_response, '$..deeplinkflag', "返回的是否deeplink与配置不一致",
                         "mango配置了deeplinkflag但实际未下发")
